...
String res_feeds = "";                  // Response from the ThingSpeak
int ret_Len;                            // Length of the response
int pos;                                // Position of the field
unsigned int delayVal = 15000;          // Delay value
String messager;                        // Message to be sent
String x01;                             // Value of the alert field

int currentMillis;                      // [NEW 1.] Current time     
int previousMillis = 0;                 // [NEW 2.] Previous time
const unsigned long timer = 600000;     // [NEW 3.] Every 10 minutes the traffic light is set out of service
const unsigned long duration = 60000;   // [NEW 4.] For 1 minute the traffic light is set out of service
...
/*
 *  Task C.2 : The setting that updates the field8 with the value 1 (OUT OF SERVICE) every 10 minutes and for a duration of 1 minute
 */
    ...
    { 
		// Set the value 1 to the field8 field
        previousMillis = currentMillis;
        Serial.println("Setting the ALERT field to 1.");
        setFieldValue(fieldAlert, myWriteAPI, 1);
		
		// Set the traffic light to out of service state
		Serial.println("ALERT: Traffic Light out of order, setting the ORANGE signal.");
        setTrafficLight("ORANGE");
		
		// For 1 minute the traffic light is set out of service
		delay(duration);
		
		// The traffic light is set back to service
		setFieldValue(fieldAlert, myWriteAPI, 0);
    }
...
// Function for setting the traffic light to a specific color
...
// Function for writing a value to a field in ThingSpeak
...
// Function for reading the value of a field in ThingSpeak
String getFieldValue(String field) 
{
	...
	// Read the response of the GET request, in order to extract the value of field8 from the feeds.JSON of the channel
